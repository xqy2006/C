...
class API {
  constructor(options) {
    ...
    this.ready = this.memfs.ready.then(
        async () => { 
                ...
                await this.hostLogAsync(`正在加载 ${this.clangFilename}`,this.compileStreaming(this.clangFilename)).then((module123)=>{this.moduleCache[this.clangFilename] = module123;});
                await this.hostLogAsync(`正在加载 ${this.lldFilename}`,this.compileStreaming(this.lldFilename)).then((module123)=>{this.moduleCache[this.lldFilename] = module123;});
  ...
  async getModule(name) {
    ...
    const module = await this.hostLogAsync(`正在加载 ${name}`,
  ...
  async untar(memfs, filename) {
    ...
    await this.hostLogAsync(`正在解压 ${filename}`, promise);
  }
  ...
  async compileToAssembly(options) {
    ...
    await this.ready;
    this.memfs.addFile(input, contents);
    const clang = await this.getModule(getModulegetModule);
  ...
  async compileLinkRun(contents) {
    ...
    const testMod = await this.hostLogAsync(`编译中`,
