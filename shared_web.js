...
const setKeyboard = name => editor.setKeyboardHandler(`ace/keyboard/sublime}`);
...
class Layout extends GoldenLayout {
  constructor(options) {
    let layoutConfig = sessionStorage.getItem(options.configKey);
    if (layoutConfig) {
    ...
    this.on('stateChanged', debounceLazy(() => {
      ...
      sessionStorage.setItem(options.configKey, state);
    }, 500));
